    ...
    <AnswerContainer>
      <AnswerList>
        {answers.map((answer, index) => (
          <AnswerItem key={index}>
            ...
            <div>
              {editingIndex === index ? (
                <AnswerButton onClick={handleAnswerSubmit}>Save</AnswerButton>
              ) : (
                <AnswerEditButton
                  onClick={() => handleAnswerEdit(index, answer)}
                >
                  Edit
                </AnswerEditButton>
              )}
              <AnswerDeleteButton onClick={() => handleAnswerDelete(index)}>
                Delete
              </AnswerDeleteButton>
            </div>
          </AnswerItem>
      ...
      <AnswerForm onSubmit={handleAnswerSubmit}>
        ...
        <AnswerButton type="submit">
          {editingIndex !== -1 ? "Save Edit" : "Post Your Answer"}
        </AnswerButton>
      </AnswerForm>
    </AnswerContainer>
  );
